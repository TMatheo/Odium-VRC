...
namespace Odium.QMPages
{
	...
	{
		// Token: 0x060000FB RID: 251 RVA: 0x00008D30 File Offset: 0x00006F30
		public static void InitializePage(QMNestedMenu gameHacks, Sprite buttonImage)
		{
			Sprite sprite = SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\Murder.png", 100f);
			Sprite icon = SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\Kill.png", 100f);
			Sprite sprite2 = SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\Win.png", 100f);
			Sprite sprite3 = SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\People.png", 100f);
			Sprite sprite4 = SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\Gun.png", 100f);
			Sprite sprite5 = SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\WorldIcon.png", 100f);
			Sprite sprite6 = SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\FTAC.png", 100f);
			QMNestedMenu location = new QMNestedMenu(gameHacks, 1f, 0f, "<color=#8d142b>Murder 4</color>", "<color=#8d142b>Murder 4</color>", "Opens Select User menu", false, sprite, buttonImage);
			QMNestedMenu btnMenu = new QMNestedMenu(location, 1f, 0f, "<color=#8d142b>Win Triggers</color>", "<color=#8d142b>Win Triggers</color>", "Opens Select User menu", false, sprite2, buttonImage);
			QMNestedMenu btnMenu2 = new QMNestedMenu(location, 2f, 0f, "<color=#8d142b>Player Actions</color>", "<color=#8d142b>Player Actions</color>", "Opens Select User menu", false, sprite3, buttonImage);
			QMNestedMenu qmnestedMenu = new QMNestedMenu(location, 3f, 0f, "<color=#8d142b>World Actions</color>", "<color=#8d142b>World Actions</color>", "Opens Select User menu", false, sprite5, buttonImage);
			QMNestedMenu btnMenu3 = new QMNestedMenu(location, 4f, 0f, "<color=#8d142b>Gun Actions</color>", "<color=#8d142b>Gun Actions</color>", "Opens Select User menu", false, sprite4, buttonImage);
			QMNestedMenu qmnestedMenu2 = new QMNestedMenu(location, 2.5f, 1f, "<color=#8d142b>Exploits</color>", "<color=#8d142b>Exploits</color>", "Opens Select User menu", false, SpriteUtil.LoadFromDisk(Environment.CurrentDirectory + "\\Odium\\ExploitIcon.png", 100f), buttonImage);
			new QMSingleButton(qmnestedMenu2, 2f, 1.5f, "Crash All", delegate()
			{
				try
				{
					...
					MelonCoroutines.Start(GameHacks.CrashPlayersWithDelay(targets));
				}
				catch (Exception ex)
				{
				}
			}, "Brings death to all players", false, icon, buttonImage);
			...
			}, "Brings death to all players", false, sprite2, buttonImage);
			...
			QMNestedMenu btnMenu4 = new QMNestedMenu(gameHacks, 2f, 0f, "<color=#8d142b>FTAC</color>", "<color=#8d142b>FTAC</color>", "Opens Select User menu", false, sprite6, buttonImage);
		...
		// Token: 0x060000FC RID: 252 RVA: 0x000094A3 File Offset: 0x000076A3
		public static IEnumerator CrashPlayersWithDelay(List<Player> targets)
		{
			GameHacks.<CrashPlayersWithDelay>d__1 <CrashPlayersWithDelay>d__ = new GameHacks.<CrashPlayersWithDelay>d__1(0);
			<CrashPlayersWithDelay>d__.targets = targets;
			return <CrashPlayersWithDelay>d__;
		}
	}
}
